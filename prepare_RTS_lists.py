...
def prep_list(self, data_dir, save_dir, split):
    '''
    args:
    ...
    for i in range(1, 9):
        names_list = []
        data_dir_i = data_dir + 'instrument_dataset_' + i + '/images/'
        ...
        with open(save_dir + split, 'w') as fp:
            for n in names_list:
                fp.write("%s\n" % n)
            print("[INFO] Added instrument_dataset_{} {} into {}", i, split, save_dir)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("./lists/lists_RTS_binary"):
        os.makedirs("./lists/lists_RTS_binary")
    ...
    if not os.path.exists("./lists/lists_RTS_instrument"):
        os.makedirs("./lists/lists_RTS_instrument")
    ...
    if not os.path.exists("./lists/lists_RTS_parts"):
        os.makedirs("./lists/lists_RTS_parts")
    ...
    for t in task_list:
        ...
        prep_list(data_dir, save_dir, "train")
        prep_list(data_dir, save_dir, "test") 
